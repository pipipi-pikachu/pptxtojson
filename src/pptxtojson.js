...
async function getUsedFonts(zip) {
  ...
  const embeddedFontList = getTextByPathList(content, [
    "p:presentation",
    "p:embeddedFontLst",
    "p:embeddedFont",
  ]);
  ...
  const embeddedFonts =
    embeddedFontList.constructor === Array
      ? embeddedFontList
      : [embeddedFontList];
  for (const embeddedFont of embeddedFonts) {
    const typeface = getTextByPathList(embeddedFont, [
      "p:font",
      "attrs",
      "typeface",
    ]);
...
async function processSingleSlide(
  ...
  for (const relationshipArrayItem of relationshipArray) {
    ...
    // let relTarget = relationshipArrayItem["attrs"]["Target"].replace(
    //   "../",
    //   "ppt/"
    // );
    const isExternal =
      relationshipArrayItem["attrs"]["TargetMode"] === "External";
    if (!isExternal) {
...
function getNote(noteContent) {
  ...
  for (const spNode of spNodes) {
    const phType = getTextByPathList(spNode, [
      "p:nvSpPr",
      "p:nvPr",
      "p:ph",
      "attrs",
  ...
  function processNestedGroupElements(elements, ws, hs, depth = 0) {
    ...
    return elements.map((element) => {
      ...
      if (element.type === "group" && element.elements) {
        processed.elements = processNestedGroupElements(
          element.elements,
          ws,
          hs,
          depth + 1
...
async function genShape(
  ...
  if (shapType) {
    ...
    if (shapAdjst_ary) {
      const adjList = Array.isArray(shapAdjst_ary)
        ? shapAdjst_ary
        : [shapAdjst_ary];
...
async function processPicNode(node, warpObj, source) {
  ...
  let geom = "rect";
  const prstGeom = getTextByPathList(node, [
    "p:spPr",
    "a:prstGeom",
    "attrs",
    "prst",
...
async function genDiagram(node, warpObj) {
  ...
  const dgmDrwSpArray = getTextByPathList(diagramWarpObj["digramFileContent"], [
    "p:drawing",
    "p:spTree",
    "p:sp",
  ]);
  ...
  let textList = [];
  if (dgmDrwSpArray) {
    const spList = Array.isArray(dgmDrwSpArray)
      ? dgmDrwSpArray
      : [dgmDrwSpArray];
